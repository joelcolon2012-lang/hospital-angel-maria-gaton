...
export const PatientCard: React.FC<Props> = ({ patient, onSelect, onDelete }) => {
  ...
  return (
    <div
      ...
      className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-all border border-slate-200/90 active:scale-[0.99] cursor-pointer relative overflow-hidden group/card"
    >
      ...
      {/* Top row: Code, Triage & Bed + Delete */}
        ...
        <div className="flex items-center gap-1.5">
          ...
          {onDelete && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (window.confirm(`¿Está seguro de eliminar a ${patient.fullName}? Esta acción removerá el paciente de la lista activa.`)) {
                  onDelete(patient.id);
                }
              }}
